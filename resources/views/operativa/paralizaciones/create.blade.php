...
@section('title', 'Nueva Paralización')
@section('page_title', 'Nueva Paralización')
@section('page_subtitle', 'Gestión Operativa · Registrar paralización')

@section('content')
<div class="container-fluid" style="max-width:700px">
    <h3 class="fw-light text-secondary mb-4">Gestión Operativa &rsaquo; Nueva Paralización</h3>
    <form action="{{ route('operativa.paralizaciones.store') }}" method="POST">
        @csrf
        <div class="row g-3">
            <div class="col-12">
            ...
            <div class="col-12">
                ...
                <input type="text" name="motivo" class="form-control @error('motivo') is-invalid @enderror"
                       value="{{ old('motivo') }}" required maxlength="200">
            ...
            <div class="col-md-6">
                ...
                <input type="date" name="fecha_inicio_par" class="form-control @error('fecha_inicio_par') is-invalid @enderror"
                       value="{{ old('fecha_inicio_par') }}" required>
            ...
            <div class="col-md-6">
                ...
                <select name="estado" class="form-select @error('estado') is-invalid @enderror" required>
                    @foreach(['activa','levantada','en_revision'] as $est)
                        <option value="{{ $est }}" {{ old('estado','activa') == $est ? 'selected' : '' }}>{{ ucfirst(str_replace('_',' ',$est)) }}</option>
                    @endforeach
                </select>
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="registrado_por" class="form-control" value="{{ old('registrado_por', Auth::user()->nombre_usuario ?? '') }}" maxlength="100">
            </div>
